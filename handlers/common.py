...
from database.models import User, Resume, Vacancy, Application
...
async def cmd_reset(message: Message, session: AsyncSession, state: FSMContext):
    await state.clear()
    user_id = message.from_user.id
    
    # Удаляем всё связанное с пользователем
    # Сначала отклики, потом остальное (из-за связей)
    await session.execute(delete(Application).where(Application.resume_id.in_(
        select(Resume.id).where(Resume.user_id == user_id)
    )))
    await session.execute(delete(Resume).where(Resume.user_id == user_id))
    await session.execute(delete(Vacancy).where(Vacancy.employer_id == user_id))
    await session.execute(delete(User).where(User.tg_id == user_id))
    
    await session.commit()
    
    await message.answer("♻️ <b>Все ваши данные удалены. Вы можете начать заново!</b>\n\nВведите /start для запуска.", parse_mode="HTML")

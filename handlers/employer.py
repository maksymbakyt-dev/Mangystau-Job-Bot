...
async def cmd_my_vacancies(message: Message, session: AsyncSession):
    ...
    for v in vacancies:
        ...
        # Кнопка для просмотра списка откликнувшихся
        markup = None
        if apps:
...
async def process_view_apps(callback: CallbackQuery, session: AsyncSession):
    ...
    for app in apps:
        ...
        seeker_text = (f"🎉 <b>Новый отклик на вакансию '{vacancy.title}'!</b>\n\n"
                       f"👤 Соискатель: {user_info.name}\n"
                       f"🔗 Контакт: {'@' + user_info.username if user_info.username else f'<a href=\"tg://user?id={user_info.tg_id}\">Профиль</a>'}\n"
        ...
        # Показываем анкету и кнопки решения (если еще не решено)
        markup = get_employer_decision_keyboard(app.id, user_info.language) if app.status == "pending" else None
